...
int main() {
  ...
  // Create socket
  if ((server_sock = berkeley_socket(AF_INET, SOCK_STREAM, 0)) == -1) {
    perror("socket");
  ...
  // Connect
  if ((berkeley_connect(server_sock, (struct sockaddr *)&server_addr,
  ...
  while (1) {
    ...
    // Send
    sent_message_length =
        berkeley_send(server_sock, sent_message, strlen(sent_message), 0);

    // Receive
    received_message_length = berkeley_recv(server_sock, &received_message,

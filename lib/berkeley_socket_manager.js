...
export default class BerkeleySocketManager {
  ...
  constructor(getConnection, getReceiver, getAccepter, getOnListen) {
    this.#getConnection = getConnection;
    this.#getReceiver = getReceiver;
    this.#getAccepter = getAccepter;
    this.#getOnListen = getOnListen;
  }
  ...
  static Builder = class {
    ...
    setGetAccepter = (handler) => {
      this.#getAccepter = handler;

      return this;
    };

    setOnListen = (handler) => {
      this.#getOnListen = handler;

      return this;
    };

    build() {
      return new BerkeleySocketManager(
        this.#getConnection,
        this.#getReceiver,
        this.#getAccepter,
        this.#getOnListen
    ...
    return {
      ...
      berkeley_sockets_bind: async (fd, addressPointer, addressLength) => {
        ...
        await socket.bind(
          ...
          await this.#getAccepter(),
          await this.#getOnListen()
        );
      ...
      berkeley_sockets_accept: async (
        ...
        const socket = this.#sockets.get(fd);
        const { family, port, address } = await socket.accept(this.#getReceiver);

...
int main() {
  ...
  // Create socket
  if ((server_sock = berkeley_sockets_socket(AF_INET, SOCK_STREAM, 0)) == -1) {
    perror("socket");
  ...
  // Connect
  if ((berkeley_sockets_connect(server_sock, (struct sockaddr *)&server_addr,
  ...
  while (1) {
    ...
    // Send
    sent_message_length = berkeley_sockets_send(server_sock, sent_message,
                                                strlen(sent_message), 0);

    // Receive
    received_message_length = berkeley_sockets_recv(
        server_sock, &received_message, RECEIVED_MESSAGE_MAX_LENGTH, 0);
